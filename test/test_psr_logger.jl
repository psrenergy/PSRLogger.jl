...
function test_create_two_psr_loggers_in_different_files()
    ...
    psr_logger1 = PSRLog.create_psr_logger(log_path1)
    psr_logger2 = PSRLog.create_psr_logger(log_path2)
    @test isfile(log_path1)
    @test isfile(log_path2)
    PSRLog.close_psr_logger(psr_logger1)
    PSRLog.close_psr_logger(psr_logger2)
    rm(log_path1)
    rm(log_path2)
end
...
function test_create_two_psr_loggers_in_the_same_file()
    ...
    psr_logger = PSRLog.create_psr_logger(log_path)
    @test isfile(log_path)
    @test_throws ErrorException PSRLog.create_psr_logger(log_path)
    PSRLog.close_psr_logger(psr_logger)
    rm(log_path)
end
